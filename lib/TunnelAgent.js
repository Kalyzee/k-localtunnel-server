...
class TunnelAgent extends Agent {
    ...
    _onConnection(socket) {
        ...
        socket.once('data', chunk => {
            ...
            const match = buffer.match(/x-lt-auth:\s*(\S+)/);
            if (!match || (this.token && this.token !== match[1])) {
                this.debug('Token invalide, fermeture du socket');
                socket.destroy();
                return;
            }

            this.debug('Token valide, connexion TCP acceptée');
        });
