...
class Client extends EventEmitter {
    constructor(options) {
        ...
        this.tcpAgent = options.tcpAgent || null;
        this.udpAgent = options.udpAgent || null;
        ...
        this.graceTimeout = setTimeout(() => {
            this.debug('grace timeout expired, closing client %s', id);
            this.close();
        }, this.gracePeriod).unref();
        ...
        agent.on('offline', () => {
            ...
            this.graceTimeout = setTimeout(() => {
                this.debug('reconnect grace timeout expired, closing client %s', id);
                this.close();
            }, this.gracePeriod).unref();
        });
    ...
    stats() {
        const agentStats = this.agent.stats();
        if (this.tcpAgent) {
            const tcpStats = this.tcpAgent.stats();
            agentStats.activeExternalConnections = tcpStats.activeExternalConnections;
        }
        if (this.udpAgent) {
            const udpStats = this.udpAgent.stats();
            agentStats.activeSessions = udpStats.activeSessions;
        }
        return agentStats;
    }
    ...
    close() {
        clearTimeout(this.graceTimeout);
        this.tcpAgent?.destroy();
        this.udpAgent?.destroy();
        this.agent.destroy();
        this.emit('close');
    }
